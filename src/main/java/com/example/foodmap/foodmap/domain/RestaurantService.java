...
public class RestaurantService {
	...
	public RestaurantService(RestaurantRepository restaurantRepository, RestaurantPhotoRepository photoRepository,
			RestaurantReviewRepository reviewRepository, RestaurantFavoriteRepository favoriteRepository,
			MemberRepository memberRepository, RestaurantHourRepository hourRepository,
	...
	// ===== 搜尋（保留） =====
	public Page<RestaurantDto> searchRestaurants(String county, Double minRating, String type, Pageable pageable,
			Long memberId) {
		...
		if (hasCounty && hasType) {
			// 有縣市 + 類別 -> 盡量帶上評分門檻
			if (min != null && min > 0) {
				page = restaurantRepository.findByStatusAndCountyAndRatingGreaterThanEqualAndTypeContainingIgnoreCase(
						ModerationStatus.APPROVED, county, min, type, pageable);
			} else {
				page = restaurantRepository.findByStatusAndCountyAndTypeContainingIgnoreCase(ModerationStatus.APPROVED,
						county, type, pageable);
			}
		} else if (hasCounty) {
			page = restaurantRepository.findByStatusAndCounty(ModerationStatus.APPROVED, county, pageable);
		} else if (hasType) {
			page = restaurantRepository.findByStatusAndTypeContainingIgnoreCase(ModerationStatus.APPROVED, type,
					pageable);
		} else {
			// 沒任何條件就回空頁，避免把所有店抓出來
	...
	public Page<RestaurantDto> searchByKeyword(String keyword, Pageable pageable, Long memberId) {
		Page<Restaurant> page = restaurantRepository.searchApprovedByKeyword(keyword, ModerationStatus.APPROVED,
				pageable);
		return convertToDtoPage(page, memberId);
	}
	...
	public Page<RestaurantDto> searchByCountyAndType(String county, String type, Pageable pageable, Long memberId) {
		Page<Restaurant> page = restaurantRepository
				.findByStatusAndCountyAndTypeContainingIgnoreCase(ModerationStatus.APPROVED, county, type, pageable);
		return convertToDtoPage(page, memberId);
	}
	...
	private Page<RestaurantDto> convertToDtoPage(Page<Restaurant> page, Long memberId) {
		List<RestaurantDto> dtoList = page.getContent().stream().map(r -> {
			...
			return new RestaurantDto(r.getId(), r.getName(), r.getAddress(), r.getPhone(), r.getCounty(), r.getType(),
					r.getAvgRating(), thumbnail, isFav, uploaderName, status, rejectReason);
	...
	private String getUploaderNickname(Integer memberId) {
		if (memberId == null)
			return "匿名";
		return memberRepository.findById(memberId).map(Member::getMemberNickName).orElse("匿名");
	}
	...
	public boolean toggleFavorite(Long restaurantId, Long memberId) {
		...
		return favoriteRepository.findByRestaurantIdAndMemberId(restaurantId, memberId).map(existing -> {
			// 已收藏 → 取消收藏
	...
	public Restaurant getRestaurantById(Long id) {
		return restaurantRepository.findById(id).orElseThrow(() -> new RuntimeException("餐廳不存在"));
	}
	...
	public RestaurantDetailsDTO getRestaurantDetails(Long restaurantId, Long memberId) {
	    ...
	    // 只要不是已上架，就要做權限檢查
	    if (restaurant.getStatus() != ModerationStatus.APPROVED) {
	        boolean allow = false;

	        if (memberId != null) {
	            // 上傳者本人
	            if (restaurant.getSubmittedBy() != null
	                    && restaurant.getSubmittedBy().equals(memberId)) {
	                allow = true;
	            } else {
	                // 管理員
	                Member m = memberRepository.findById(memberId.intValue()).orElse(null);
	                if (m != null && m.getMemberRole() == MemberRole.ADMIN) {
	                    allow = true;
	                }
	            }
	        }
		...
		List<ReviewDto> reviewDtos = reviews.stream()
				.map(rv -> new ReviewDto(rv.getId(), rv.getMemberId() == null ? null : rv.getMemberId().longValue(),
						getUploaderNickname(rv.getMemberId()), rv.getRating(), rv.getComment(), rv.getCreatedTime(), // 傳
																														// LocalDateTime
		...
		if (specOpt.isPresent()) {
			...
			if (todays.isEmpty()) {
				...
				todayRangeText = todays.stream().map(h -> formatRange(h.getOpenTime(), h.getCloseTime()))
	...
	private void setAverageRating(Restaurant restaurant) {
		...
		if (!reviews.isEmpty()) {
			double avg = reviews.stream().mapToInt(RestaurantReview::getRating).average().orElse(0.0);
	...
	public List<Restaurant> getTopRestaurantsByAvgRating(int limit) {
		...
		return all.stream().sorted((a, b) -> Double.compare(b.getAvgRating(), a.getAvgRating())).limit(limit)
				.collect(Collectors.toList());
	}
	...
	public boolean isClosedAt(Long restaurantId, LocalDateTime when) {
		...
		boolean open = todays.stream().anyMatch(h -> h.getOpenTime() != null && h.getCloseTime() != null
	...
	public List<RestaurantSpecialHour> listSpecialHours(Long restaurantId, LocalDate from, LocalDate to) {
		return specialHourRepository.findByRestaurantIdAndSpecificDateBetweenOrderBySpecificDateAsc(restaurantId, from,
				to);
	}
	...
	public void upsertSpecialHour(RestaurantSpecialHour spec) {
		var opt = specialHourRepository.findByRestaurantIdAndSpecificDate(spec.getRestaurantId(),
				spec.getSpecificDate());
		if (opt.isPresent()) {
			var exist = opt.get();
	...
	public String guessImageContentType(byte[] data) {
		...
		if (data[0] == 'R' && data[1] == 'I' && data[2] == 'F' && data[3] == 'F' && data[8] == 'W' && data[9] == 'E'
				&& data[10] == 'B' && data[11] == 'P')
	...
	public Page<ReviewDto> getReviewPage(Long restaurantId, int page, int size, String sortBy, String order,
		...
		Page<RestaurantReview> raw = includeHidden ? reviewRepository.findByRestaurantId(restaurantId, pr)
				: reviewRepository.findByRestaurantIdAndIsHiddenFalse(restaurantId, pr);

		return raw.map(rv -> new ReviewDto(rv.getId(), rv.getMemberId() == null ? null : rv.getMemberId().longValue(),
				getUploaderNickname(rv.getMemberId()), rv.getRating(), rv.getComment(), rv.getCreatedTime(),
				Boolean.TRUE.equals(rv.getIsHidden())));
	}
	...
	public ReviewDto createReview(Long restaurantId, Long memberId, int rating, String comment) {
		if (memberId == null)
			throw new IllegalArgumentException("未登入或缺少會員資訊");
	...
	public ReviewDto createReview(Long restaurantId, Integer memberId, int rating, String comment, Boolean hidden) {
		...
		if (exists)
			throw new IllegalStateException("你已對此餐廳發表過評論");
		...
		return new ReviewDto(rv.getId(), memberId.longValue(), getUploaderNickname(rv.getMemberId()), rv.getRating(),
				rv.getComment(), rv.getCreatedTime(), Boolean.TRUE.equals(rv.getIsHidden()));
	}
	...
	public boolean updateReview(Long reviewId, Long memberId, Integer rating, String comment) {
		...
		if (rv.getMemberId() == null || rv.getMemberId().intValue() != memberId.intValue())
			return false;
	...
	private List<PhotoDto> buildPhotoDtos(Long restaurantId) {
		return photoRepository.findTop5ByRestaurantIdOrderByIdAsc(restaurantId).stream()
				...
				.map(p -> new PhotoDto(p.getId(), "/api/restaurants/photos/" + p.getId())).collect(Collectors.toList());
	}
	...
	private LinkedHashMap<DayOfWeek, List<String>> buildWeeklyHours(Long restaurantId) {
		...
		DayOfWeek[] order = { DayOfWeek.MONDAY, DayOfWeek.TUESDAY, DayOfWeek.WEDNESDAY, DayOfWeek.THURSDAY,
		...
		for (DayOfWeek d : order) {
			...
			List<String> ranges = hours.stream()
					...
					.map(h -> formatRange(h.getOpenTime(), h.getCloseTime())).collect(Collectors.toList());
			map.put(d, ranges);
		}
		return map;
	}
