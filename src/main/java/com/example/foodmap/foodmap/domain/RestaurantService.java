...
public class RestaurantService {
    ...
    public Page<RestaurantDto> searchRestaurants(String county, Double minRating, String type, Pageable pageable, Long memberId) {
        ...
        if (hasCounty && hasType) {
            if (min != null && min > 0) {
                ...
                page = restaurantRepository.findByStatusAndCountyAndTypeContainingIgnoreCase(
                        ModerationStatus.APPROVED, county, type, pageable);
            }
        } else if (hasCounty) {
            page = restaurantRepository.findByStatusAndCounty(ModerationStatus.APPROVED, county, pageable);
        } else if (hasType) {
            page = restaurantRepository.findByStatusAndTypeContainingIgnoreCase(ModerationStatus.APPROVED, type, pageable);
        } else {
    ...
    public Page<RestaurantDto> searchByKeyword(String keyword, Pageable pageable, Long memberId) {
        Page<Restaurant> page = restaurantRepository.searchApprovedByKeyword(keyword, ModerationStatus.APPROVED, pageable);
        return convertToDtoPage(page, memberId);
    }

    public Page<RestaurantDto> searchByCountyAndType(String county, String type, Pageable pageable, Long memberId) {
        Page<Restaurant> page = restaurantRepository.findByStatusAndCountyAndTypeContainingIgnoreCase(
                ModerationStatus.APPROVED, county, type, pageable);
        return convertToDtoPage(page, memberId);
    }
    ...
    private Page<RestaurantDto> convertToDtoPage(Page<Restaurant> page, Long memberId) {
        List<RestaurantDto> dtoList = page.getContent().stream().map(r -> {
            ...
            return new RestaurantDto(
                    ...
                    r.getCounty(), r.getType(), r.getAvgRating(), thumbnail,
    ...
    public boolean toggleFavorite(Long restaurantId, Long memberId) {
        ...
        return favoriteRepository.findByRestaurantIdAndMemberId(restaurantId, memberId)
                .map(existing -> {
                    favoriteRepository.delete(existing); // 取消收藏
                    return false;
                })
                .orElseGet(() -> {
    ...
    public RestaurantDetailsDTO getRestaurantDetails(Long restaurantId, Long memberId) {
        ...
        if (restaurant.getStatus() != ModerationStatus.APPROVED) {
            ...
            if (memberId != null) {
                if (restaurant.getSubmittedBy() != null && restaurant.getSubmittedBy().equals(memberId)) {
                    allow = true; // 上傳者本人
                } else {
            ...
            if (!allow) throw new IllegalStateException("FORBIDDEN");
        }
    ...
    public boolean isClosedAt(Long restaurantId, LocalDateTime when) {
        ...
        boolean open = todays.stream().anyMatch(h ->
                h.getOpenTime() != null && h.getCloseTime() != null && within(time, h.getOpenTime(), h.getCloseTime()));
        return !open;
    }

    @Transactional(readOnly = true)
    public List<RestaurantSpecialHour> listSpecialHours(Long restaurantId, LocalDate from, LocalDate to) {
        return specialHourRepository.findByRestaurantIdAndSpecificDateBetweenOrderBySpecificDateAsc(restaurantId, from, to);
    }
    ...
    public void replaceWeeklyHours(Long restaurantId, List<RestaurantHour> newHours) {
        ...
        if (newHours != null && !newHours.isEmpty()) {
            for (var h : newHours) h.setRestaurantId(restaurantId);
    ...
    public void upsertSpecialHour(RestaurantSpecialHour spec) {
        var opt = specialHourRepository.findByRestaurantIdAndSpecificDate(spec.getRestaurantId(), spec.getSpecificDate());
        if (opt.isPresent()) {
            var exist = opt.get();
    ...
    public String guessImageContentType(byte[] data) {
        ...
        if (data[0] == 'R' && data[1] == 'I' && data[2] == 'F' && data[3] == 'F' && data[8] == 'W' && data[9] == 'E' && data[10] == 'B' && data[11] == 'P') return "image/webp"; // WEBP
        return "image/jpeg";
    }

    // 分頁取得評論
    public Page<ReviewDto> getReviewPage(Long restaurantId, int page, int size, String sortBy, String order, boolean includeHidden) {
        if (page < 0) page = 0;
    ...
    public ReviewDto createReview(Long restaurantId, Integer memberId, int rating, String comment, Boolean hidden) {
        if (restaurantId == null || memberId == null) throw new IllegalArgumentException("restaurantId / memberId 不可為空");
        if (rating < 1 || rating > 5) throw new IllegalArgumentException("評分需介於 1~5 星");
        if (comment == null || comment.isBlank()) throw new IllegalArgumentException("comment 不可為空");
        ...
        rv.setMemberId(memberId); // entity 欄位是 Integer
        rv.setRating(rating);
    ...
    public boolean updateReview(Long reviewId, Long memberId, Integer rating, String comment) {
        ...
        reviewRepository.save(rv); // ✅ 需要 save

        // 重算平均星等
        restaurantRepository.findById(rv.getRestaurantId()).ifPresent(r -> {
            setAverageRating(r);
            restaurantRepository.save(r);
        });
    ...
    public boolean deleteReviewByIdAndMemberId(Long reviewId, Long memberId) {
        ...
        Long restaurantId = rv.getRestaurantId();
    ...
    /** ✅ 管理員隱藏/取消隱藏留言（記得在 Controller 驗權限） */
    @Transactional
